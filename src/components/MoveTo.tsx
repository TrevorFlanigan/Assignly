import {
  finishMoveTo,
  highestChildrenHelper,
  selectFileSystem,
} from "@/redux/fileSystemSlice";
import { useAppDispatch, useAppSelector } from "@/redux/store";
import {
  ActionIcon,
  Button,
  Container,
  Group,
  Text,
  Title,
  Tooltip,
} from "@mantine/core";
import { IconArrowLeft, IconArrowLeftBar, IconArrowRight } from "@tabler/icons";
import { useMemo, useState } from "react";

type MoveToProps = {
  srcPath?: string[];
};

const MoveTo = ({ srcPath }: MoveToProps) => {
  const fileSystem = useAppSelector(selectFileSystem);
  const [localPath, setLocalPath] = useState(srcPath?.slice(0, -1) || []);
  const [selectedPath, setSelectedPath] = useState<string[] | undefined>();
  const dispatch = useAppDispatch();
  const currDir = useMemo(() => {
    return highestChildrenHelper(fileSystem, localPath);
  }, [localPath]);
  const handleFinish = (destPath: string[]) => {
    if (srcPath) dispatch(finishMoveTo({ destPath, srcPath }));
  };

  if (!srcPath) return <></>;
  return (
    <div>
      <>
        <Group>
          <ActionIcon
            style={{ visibility: localPath.length ? "visible" : "hidden" }}
            onClick={() => {
              setSelectedPath((old) => old?.slice(0, -1) || []);
              setLocalPath((old) => old.slice(0, -1));
            }}
          >
            <IconArrowLeft />
          </ActionIcon>
          <Title order={6}>Move To</Title>
        </Group>
        <div style={{ display: "flex", flexDirection: "column" }}>
          {Object.keys(currDir).length === 0 && (
            <Text>There&apos;s nothing here.</Text>
          )}
          {Object.keys(currDir).map((key) => {
            if (srcPath.join("/") === [...localPath, key].join("/"))
              return (
                <Text my={2} color={"gray"} key={key}>
                  {key}
                </Text>
              );
            if (selectedPath?.join("/") === [...localPath, key].join("/"))
              return (
                <Group my={2} position="apart" key={key}>
                  <Button
                    style={{ flex: 1 }}
                    variant="filled"
                    key={key}
                    onClick={() => {
                      setSelectedPath([...localPath, key]);
                    }}
                  >
                    <Title order={6}>{key}</Title>
                  </Button>
                  <Tooltip label={`Go to ${key}`}>
                    <ActionIcon
                      onClick={() => {
                        setSelectedPath([...localPath, key]);
                        setLocalPath((old) => [...old, key]);
                      }}
                    >
                      <IconArrowRight />
                    </ActionIcon>
                  </Tooltip>
                </Group>
              );
            return (
              <Group my={2} position="apart" key={key}>
                <Button
                  style={{ flex: 1 }}
                  variant="subtle"
                  key={key}
                  onClick={() => {
                    setSelectedPath([...localPath, key]);
                  }}
                >
                  <Title order={6}>{key}</Title>
                </Button>
                <Tooltip label={`Go to ${key}`}>
                  <ActionIcon
                    onClick={() => {
                      setSelectedPath([...localPath, key]);
                      setLocalPath((old) => [...old, key]);
                    }}
                  >
                    <IconArrowRight />
                  </ActionIcon>
                </Tooltip>
              </Group>
            );
          })}
        </div>
        <Group mt={10} position="right">
          <Tooltip
            disabled={!selectedPath}
            label={`Move ${srcPath.slice(-1)} to ${
              selectedPath?.length ? selectedPath?.slice(-1) : "Home"
            }`}
          >
            <Button
              onClick={() => {
                if (selectedPath) handleFinish(selectedPath);
              }}
              disabled={!selectedPath}
            >
              {Object.keys(currDir).length === 0 ? "Move Here" : "Move"}
            </Button>
          </Tooltip>
        </Group>
      </>
    </div>
  );
};

export default MoveTo;
